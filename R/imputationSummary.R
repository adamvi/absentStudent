#' @title A function for pooling imputed scale scores, SGPs and Baseline SGPs
#' @description Function produces summary tables and multiple imputation
#'  statistics for imputed score data and SGP analyses.
#' @param data.to.summarize An dataset of imputed scale scores and SGPs (cohort
#'  and baseline referenced) calculated using those imputed scores.
#' @param institution.level Institution ID for summary table aggregation.
#'  E.g., "SCHOOL_NUMBER" or "DISTRICT_NUMBER". \code{NULL} (default) aggregates
#'  across all students (entire state).
#'  Default: NULL
#' @param summary.level Additional aggregation levels. E.g., "GRADE" and/or "CONTENT_AREA".
#'  Default: NULL
#' @param standardize.scores If not \code{NULL} (default), then level by which
#'  scale scores are standardized (by mean and SD of the observed scores). E.g.,
#'  "GRADE" to standardize by grade for cross grade comparisons.
#'  Default: NULL
#' @return Function returns a list including a summary table disaggregated by
#'  the requested factors and a table including the overall mean differences
#'  and correlations between the observed and (averaged/pooled) imputed values
#'  (i.e. mean SGPs and scale scores). The summary table includes several statistics
#'  derived for the evaluation of multiple imputation analyses.
#' @details Produces summary tables and multiple imputation statistics. Function
#'  assumes data contains variables named "SCALE_SCORE_OBSERVED", "SGP_OBSERVED",
#'  and "SGP_BASELINE_OBSERVED" for the unaltered (missing) data, and imputed/imputation
#'  derived variables with the tag \code{_IMPUTED_} added. For example,
#'  "SCALE_SCORE_IMPUTED_1", "SGP_IMPUTED_1", and "SGP_BASELINE_IMPUTED_1".
#'  That is, the data is in a wide format (not stacked by imputation).
#' @rdname imputationSummary
#' @export 
...
imputationSummary =
  ...
  smry_imp <- data.table::data.table()
  for (IMP in seq(M.IMP)) {
    ...
    tmp_imp_smry <- data.to.summarize[, list(
        ...
        SS_U_Imp = stats::var(get(ss.imp), na.rm = TRUE),  #  Ubar_l_SS

        Mean_SGP_Imputed = mean(get(sgp.imp), na.rm = TRUE),  #  Q_l_SGP
        SGP_U_Imp = stats::var(get(sgp.imp), na.rm = TRUE),  #  Ubar_l_SGP

        Mean_SGPB_Imputed = mean(get(sgpb.imp), na.rm = TRUE),  #  Q_l_SGP_Baseline
        SGPB_U_Imp = stats::var(get(sgpb.imp), na.rm = TRUE)),  #  Ubar_l_SGP_Baseline
      keyby = c(institution.level, summary.level)][, IMP_N := IMP]

      smry_imp <- data.table::rbindlist(list(smry_imp, tmp_imp_smry))
  }
  ...
  smry_all[!is.na(SS_ADF) & SS_ADF != 0, SS_tStat := stats::qt(0.975, df=SS_ADF)*(sqrt(SS_T))]
  smry_all[!is.na(SGP_ADF) & SGP_ADF != 0, SGP_tStat := stats::qt(0.975, df=SGP_ADF)*(sqrt(SGP_T))]
  smry_all[!is.na(SGPB_ADF) & SGPB_ADF != 0, SGPB_tStat := stats::qt(0.975, df=SGPB_ADF)*(sqrt(SGPB_T))]
  ...
  smry_all[!is.na(SS_ADF) & SS_ADF != 0, SS_F_p := round(stats::pf(SS_F_Stat, df1=1, df2=SS_ADF, lower.tail=FALSE), 5)]
  smry_all[, SS_F_Simp := ((Mean_SS_Observed - Mean_SS_Imputed)^2)/((1 + 1/M.IMP)*SS_B)]
  smry_all[, SS_F_p_simp := round(stats::pf(SS_F_Simp, df1=1, df2=nu, lower.tail=FALSE), 5)]

  smry_all[, SGP_F_Stat := ((Mean_SGP_Observed - Mean_SGP_Imputed)^2)/SGP_T]
  smry_all[!is.na(SGP_ADF) & SGP_ADF != 0, SGP_F_p := round(stats::pf(SGP_F_Stat, df1=1, df2=SGP_ADF, lower.tail=FALSE), 5)]
  smry_all[, SGP_F_Simp := ((Mean_SGP_Observed - Mean_SGP_Imputed)^2)/((1 + 1/M.IMP)*SGP_B)]
  smry_all[, SGP_F_p_simp := round(stats::pf(SGP_F_Simp, df1=1, df2=nu, lower.tail=FALSE), 5)]

  smry_all[, SGPB_F_Stat := ((Mean_SGPB_Observed - Mean_SGPB_Imputed)^2)/SGPB_T]
  smry_all[!is.na(SGPB_ADF) & SGPB_ADF != 0, SGPB_F_p := round(stats::pf(SGPB_F_Stat, df1=1, df2=SGPB_ADF, lower.tail=FALSE), 5)]
  smry_all[, SGPB_F_Simp := ((Mean_SGPB_Observed - Mean_SGPB_Imputed)^2)/((1 + 1/M.IMP)*SGPB_B)]
  smry_all[, SGPB_F_p_simp := round(stats::pf(SGPB_F_Simp, df1=1, df2=nu, lower.tail=FALSE), 5)]
  ...
  overall_smry <- smry_all[, list(
      ...
      Cor_SS = round(stats::cor(Mean_SS_Observed, Mean_SS_Imputed, use="na.or.complete"), 3),

      Mean_SGP_Diff = round(mean(Mean_SGP_Observed-Mean_SGP_Imputed, na.rm=TRUE), 3),
      Cor_SGP = round(stats::cor(Mean_SGP_Observed, Mean_SGP_Imputed, use="na.or.complete"), 3),

      Mean_SGPB_Diff = round(mean(Mean_SGPB_Observed-Mean_SGPB_Imputed, na.rm=TRUE), 3),
      Cor_SGPB = round(stats::cor(Mean_SGPB_Observed, Mean_SGPB_Imputed, use="na.or.complete"), 3)
  ), keyby=summary.level]
